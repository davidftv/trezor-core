            ...
            for i in range(tx.inputs_count):
                ...
                if i == i_sign:
                    ...
                    if txi_sign.script_type == InputScriptType.SPENDMULTISIG:
                        pubkeys = [multisig_get_pubkey(hd) for hd in txi_sign.multisig.pubkeys]
                        txi_sign.script_sig = script_multisig(pubkeys, txi_sign.multisig.m)
...
def input_derive_script(i: TxInputType, pubkey: bytes, signature: bytes = None) -> bytes:
    ...
    # mutlisig
        ...
        pubkeys = [multisig_get_pubkey(hd) for hd in i.multisig.pubkeys]
        return input_script_multisig(signature, i.multisig.signatures, pubkeys, i.multisig.m)
    else:
